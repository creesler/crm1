...
const AddDetails = () => {
  ...
  const handleCostTypeChange = (e) => {
    ...
    if (costType === 0) {
      dispatch({ type: 'UPDATE_PROJECT_DETAILS', payload: { price: 0 } });
    } else {
      dispatch({ type: 'UPDATE_PROJECT_DETAILS', payload: { price: 15 } });
      }
  };
  const handlePriceChange = (e) => {
    dispatch({ type: 'UPDATE_PROJECT_DETAILS', payload: { price: e.target.value } });
  };
    return (
      <Stack
      sx={{
